...
module.exports = class MochaAdapter extends EventEmitter {
    ...
    _runTest(test, workers) {
        return this._browserAgent.getBrowser()
            ...
            .then(() => {
                ...
                const defaultOptions = browser.publicAPI.requestHandler.defaultOptions;

                return q.ninvoke(workers, 'runTest', fullTitle, {browserId: browser.id, sessionId: browser.sessionId, hostname: defaultOptions.hostname, port: defaultOptions.port})
                    .then((data) => {
    ...
    _extendTestInfo(test, data) {
        ...
        _.extend(test, {
            ...
            host: `${this._browser.publicAPI.requestHandler.defaultOptions.hostname}:${this._browser.publicAPI.requestHandler.defaultOptions.port}`,
            meta,
            hermioneCtx
        });
    }
